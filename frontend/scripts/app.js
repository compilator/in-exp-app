...
async function loadRegisterPage() {
  ...
  const oldScript = document.querySelector('script[src="scripts/reg.js"]');
  if (oldScript) oldScript.remove();

  const script = document.createElement('script');
  script.src = 'scripts/reg.js?v=' + Date.now();
  document.body.appendChild(script);
}
  ...
  container.innerHTML = categories.map(cat => `
    <div class="category-item" data-id="${cat.id}">
      ...
      <div class="category-actions">
        <button class="edit-btn" onclick="router.navigate('/${type}/edit/${cat.id}')">
          <img src="images/pen.png" alt="Редактировать">
        </button>
        <button class="delete-btn" onclick="deleteCategory('${type}', ${cat.id})">
...
function renderOperations(operations) {
  ...
  tbody.innerHTML = operations.map(op => `
    <tr>
      ...
      <td>${op.comment || ''}</td>
      <td>
        <button onclick="router.navigate('/${op.type}/edit/${op.id}')">
          <img src="images/pen.png" alt="Редактировать">
        </button>
